...
$complaints = $pdo->query(
    "SELECT c.id, c.message, c.created_at, i.title AS institution_name
     FROM complaints c JOIN institutions i ON c.institution_id = i.id
...
<div class="admin-layout">
    ...
    <aside class="admin-sidebar">
        ...
        <button class="admin-nav-link <?= $section === 'overview'     ? 'active' : '' ?>" onclick="showSection('overview')">
            <i class="bi bi-grid-1x2"></i> Обзор
        </button>
        <button class="admin-nav-link <?= $section === 'complaints'   ? 'active' : '' ?>" onclick="showSection('complaints')">
        ...
        <button class="admin-nav-link <?= $section === 'institutions' ? 'active' : '' ?>" onclick="showSection('institutions')">
            <i class="bi bi-building"></i> Учреждения
        </button>
        <button class="admin-nav-link <?= $section === 'suggestions'  ? 'active' : '' ?>" onclick="showSection('suggestions')">
        ...
        <button class="admin-nav-link <?= $section === 'users' ? 'active' : '' ?>" onclick="showSection('users')">
    ...
    <main>
        ...
        <div class="admin-tabs" id="adminTabs">
            <button class="admin-tab-btn <?= $section === 'overview'     ? 'active' : '' ?>" onclick="showSection('overview')">
                <i class="bi bi-grid-1x2"></i> Обзор
            </button>
            <button class="admin-tab-btn <?= $section === 'complaints'   ? 'active' : '' ?>" onclick="showSection('complaints')">
                <i class="bi bi-chat-left-text"></i> Жалобы
            </button>
            <button class="admin-tab-btn <?= $section === 'institutions' ? 'active' : '' ?>" onclick="showSection('institutions')">
                <i class="bi bi-building"></i> Учреждения
            </button>
            <button class="admin-tab-btn <?= $section === 'suggestions'  ? 'active' : '' ?>" onclick="showSection('suggestions')">
                <i class="bi bi-lightbulb"></i> Предложения
                <?php if ($pending_count > 0): ?>
            ...
            <?php if ($is_super): ?>
            <button class="admin-tab-btn <?= $section === 'users' ? 'active' : '' ?>" onclick="showSection('users')">
                <i class="bi bi-people"></i> Пользователи
            </button>
            <?php endif; ?>
        </div>
        ...
        <div class="admin-section <?= $section === 'complaints' ? 'active' : '' ?>" id="section-complaints">
            ...
            <div class="table-responsive-wrap">
                <table class="admin-table">
                    <thead><tr><th>#</th><th>Учреждение</th><th>Сообщение</th><th>Дата</th><th></th></tr></thead>
                    <tbody>
                            ...
                            <td>
                                <a href="scripts/admin_actions.php?action=delete_complaint&id=<?= $c['id'] ?>"
                                   ...
                                   onclick="return confirm('Удалить эту жалобу?')">
                                    <i class="bi bi-trash3"></i> <span class="d-none d-sm-inline">Удалить</span>
                                </a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
        ...
        <div class="admin-section <?= $section === 'institutions' ? 'active' : '' ?>" id="section-institutions">
            ...
            <div class="admin-add-card">
                ...
                <form action="scripts/admin_actions.php" method="POST">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        <input type="text" name="title" class="form-control" placeholder="Название учебного заведения…" required style="max-width:420px; min-width:200px;">
            ...
            <div class="table-responsive-wrap">
                <table class="admin-table">
                    <thead><tr><th>#</th><th>Название</th><th>Жалоб</th><th></th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span style="font-size:12px;color:var(--muted);" title="Сначала удалите все жалобы">
                                    <i class="bi bi-lock"></i> <span class="d-none d-sm-inline">Есть жалобы</span>
                                </span>
                                <?php endif; ?>
                            </td>
                        </tr>
        ...
        <div class="admin-section <?= $section === 'suggestions' ? 'active' : '' ?>" id="section-suggestions">
            ...
            <div class="table-responsive-wrap">
                <table class="admin-table">
                    <thead><tr><th>#</th><th>Название</th><th>Дата</th><th>Статус</th><th></th></tr></thead>
                    <tbody>
                            ...
                            <td>
                                ...
                                <div class="d-flex gap-1 flex-wrap">
                                    <a href="scripts/admin_actions.php?action=approve_suggestion&id=<?= $s['id'] ?>"
                                       ...
                                       onclick="return confirm('Одобрить и добавить «<?= htmlspecialchars(addslashes($s['title'])) ?>»?')">
                                        <i class="bi bi-check-lg"></i> <span class="d-none d-md-inline">Принять</span>
                                    </a>
                                    <a href="scripts/admin_actions.php?action=reject_suggestion&id=<?= $s['id'] ?>"
                                       class="btn-action-danger"
                                       onclick="return confirm('Отклонить это предложение?')">
                                        <i class="bi bi-x-lg"></i> <span class="d-none d-md-inline">Отклонить</span>
                                    </a>
                                </div>
                                <?php else: ?>
        ...
        <div class="admin-section <?= $section === 'users' ? 'active' : '' ?>" id="section-users">
                ...
                <form action="scripts/admin_actions.php" method="POST">
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-12 col-sm-6 col-md-3">
                            ...
                            <input type="password" name="password" class="form-control" placeholder="Мин. 6 символов" required minlength="6">
                        </div>
            ...
            <div class="table-responsive-wrap">
                <table class="admin-table">
                    <thead><tr><th>#</th><th>Имя</th><th>Логин</th><th>Роль</th><th>Добавлен</th><th></th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="role-badge role-badge-<?= $u['role'] ?>"><?= (int)$u['role'] === 1 ? 'Суперадмин' : 'Админ' ?></span></td>
...
function showSection(name) {
    ...
    document.querySelectorAll('.admin-nav-link, .admin-tab-btn').forEach(b => b.classList.remove('active'));

    const section = document.getElementById('section-' + name);
    if (section) section.classList.add('active');

    document.querySelectorAll('.admin-nav-link, .admin-tab-btn').forEach(b => {
